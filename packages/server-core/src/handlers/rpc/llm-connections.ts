...
export function registerLlmConnectionsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.settings.TEST_LLM_CONNECTION_SETUP, async (_ctx, params: import('@craft-agent/shared/protocol').TestLlmConnectionParams): Promise<import('@craft-agent/shared/protocol').TestLlmConnectionResult> => {
    ...
    try {
      ...
      return result.success ? { success: true } : { success: false, error: parseTestConnectionError(result.error || 'Unknown error') }
    } catch (error) {
      const elapsed = Date.now() - startedAt
  ...
  server.handle(RPC_CHANNELS.llmConnections.LIST_WITH_STATUS, async (): Promise<LlmConnectionWithStatus[]> => {
    ...
    return getLlmConnections().map((conn): LlmConnectionWithStatus => attachRuntimeModelState({
      ...conn,
  ...
  server.handle(RPC_CHANNELS.llmConnections.GET_API_KEY, async (): Promise<string | null> => {
    return null
  })
  ...
  server.handle(RPC_CHANNELS.llmConnections.SAVE, async (_ctx, connection: LlmConnection): Promise<{ success: boolean; error?: string }> => {
    try {
      const qwenConnection: LlmConnection = {
        ...
        name: connection.name || 'Qwen Code',
        providerType: 'qwen',
        authType: 'none',
      }
      ...
      if (!getDefaultLlmConnection()) setDefaultLlmConnection(qwenConnection.slug)
      await sessionManager.reinitializeAuth(qwenConnection.slug)
      return { success: true }
    } catch (error) {
